...
function filterProfanity($text) {
    $profanity = [
        ...
        'damn', 'bastard', 'whore', 'slut', 'nazi', 'hitler', 'nigger', 
        'japierodle', 'niger', 'franek', 'N!GURS'
    ];
...
function sanitizeInput($input, $maxLength = 1000) {
    ...
    // Usuń tagi HTML i PHP
    $input = strip_tags($input);
    
    // Usuń potencjalnie niebezpieczne ciągi
    $dangerous = [
        '<script',
        '</script>',
        'javascript:',
...
// 2. Rate limiting - dodaj do config.php:
...
function checkRateLimit($action, $limit = 10, $timeWindow = 60) {
    ...
    // Reset po przekroczeniu okna czasowego
...
// 3. W KAŻDYM pliku z formularzem - dodaj na początku:

// Zabezpieczenie przed flood
if (!checkRateLimit('form_submit', 10, 60)) {
    die('Zbyt wiele żądań. Poczekaj chwilę.');
}

// Sanitacja WSZYSTKICH inputów
$_POST = sanitizeInput($_POST);
$_GET = sanitizeInput($_GET);

// 4. Wyłącz Console w produkcji - dodaj na końcu każdej strony przed </body>:
?>

<script>
// Wyłącz console w produkcji
(function() {
    'use strict';
    ...
    // Nadpisz console (opcjonalne - może być uciążliwe w developmencie)
...
(function() {
    ...
    document.addEventListener('click', () => {
        ...
        clickTimer = setTimeout(() => {
            if (clickCount > 20) {
                suspiciousActivity++;
                console.warn('Wykryto podejrzaną aktywność: zbyt wiele kliknięć');
            }
            clickCount = 0;
        }, 1000);
    });
    
    // Jeśli zbyt wiele podejrzanych działań - zablokuj
    setInterval(() => {
        if (suspiciousActivity > 5) {
            alert('Wykryto podejrzaną aktywność. Strona zostanie przeładowana.');
...
<?php
// 5. Zabezpieczenie bazy danych - w config.php dodaj:

function getDBConnection() {
    $conn = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
    if ($conn->connect_error) {
        // NIE pokazuj szczegółów błędu w produkcji
        error_log("Database connection failed: " . $conn->connect_error);
        die("Błąd połączenia z bazą danych. Skontaktuj się z administratorem.");
    }
    $conn->set_charset("utf8mb4");
    
    // Ustaw tryb strict SQL
    $conn->query("SET sql_mode='STRICT_ALL_TABLES'");
    
    return $conn;
}

// 6. Walidacja długości nazwy produktu - w dodaj_produkt.php i edytuj_produkt.php:

if (strlen($nazwa) > 100) {
    $msg = "Nazwa produktu może mieć maksymalnie 100 znaków.";
    $msgType = "error";
}

// 7. Dodatkowe zabezpieczenie w czat.php przed spamem:

// Na początku pliku czat.php:
if (!checkRateLimit('chat_view', 30, 60)) {
    die('Zbyt wiele żądań do czatu. Poczekaj chwilę.');
}

// 8. W dodaj_produkt.php - zabezpieczenie przed wielokrotnym submitowaniem:
?>

<script>
// Zabezpieczenie formularza przed wielokrotnym wysłaniem
document.getElementById('productForm').addEventListener('submit', function(e) {
    ...
    // Timeout bezpieczeństwa - jeśli formularz się nie wyśle, odblokuj
    setTimeout(() => {
...
<?php
// 9. Dodatkowe nagłówki bezpieczeństwa - dodaj na początku każdej strony:
...
// Content Security Policy (opcjonalne - może wymagać dostosowania)
header("Content-Security-Policy: default-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdnjs.cloudflare.com https://via.placeholder.com; img-src 'self' data: https:;");

// Sanityzacja wszystkich inputów - zabezpieczenie przed XSS i złośliwym kodem
function sanitizeInput($input, $maxLength = 1000) {
    if (is_array($input)) {
        return array_map('sanitizeInput', $input);
    }
...
// Rate limiting - zapobieganie spamowi
function checkRateLimit($action, $limit = 10, $timeWindow = 60) {
    if (empty($_SESSION['user_id'])) {
        return false;
    }
