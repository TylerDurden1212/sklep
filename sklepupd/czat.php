...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['message'])) {
    // Rate limiting - max 1 wiadomość co 5 sekund
    if (!checkRateLimit('chat_send_' . $produkt_id, 1, 5)) {
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {
            header('Content-Type: application/json');
            echo json_encode(['status' => 'error', 'message' => 'Możesz wysyłać wiadomości co 5 sekund']);
            exit;
        }
        $msg_error = "Możesz wysyłać wiadomości co 5 sekund";
    } else {
        $msg = sanitizeInput(trim($_POST['message']), MAX_MESSAGE_LENGTH);
        
        // Filtracja wulgaryzmów
        if (!filterProfanity($msg)) {
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {
                header('Content-Type: application/json');
                echo json_encode(['status' => 'error', 'message' => 'Wiadomość zawiera niedozwolone słowa']);
                exit;
            }
            $msg_error = "Wiadomość zawiera niedozwolone słowa";
        } elseif ($msg !== '' && strlen($msg) <= MAX_MESSAGE_LENGTH) {
            // Sprawdź czy nie jest to powtórzona wiadomość (ostatnie 5 sekund)
            $stmt = $conn->prepare("
                SELECT message FROM chats 
                WHERE user_from = ? AND produkt_id = ? 
                AND created_at > DATE_SUB(NOW(), INTERVAL 5 SECOND)
                ORDER BY created_at DESC LIMIT 1
            ");
            $stmt->bind_param("ii", $from_id, $produkt_id);
            $stmt->execute();
            $lastMsg = $stmt->get_result()->fetch_assoc();
            $stmt->close();
            
            if ($lastMsg && $lastMsg['message'] === $msg) {
                if (!empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {
                    header('Content-Type: application/json');
                    echo json_encode(['status' => 'error', 'message' => 'Nie możesz wysyłać tej samej wiadomości ponownie']);
                    exit;
                }
                $msg_error = "Nie możesz wysyłać tej samej wiadomości ponownie";
            } else {
                // Zapisz wiadomość
    ...
    if (!empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {
        header('Content-Type: application/json');
        echo json_encode(['status' => isset($msg_error) ? 'error' : 'ok', 'message' => $msg_error ?? '']);
        exit;
    }
}
...
// W czat.php - Dodaj te funkcje JavaScript (zastąp istniejące funkcje renderNegotiationMessage i handleProfileClick)
...
function renderNegotiationMessage(msgData, messageId, isMyMessage, currentNegotiation) {
    ...
    if (msgData.type === 'price_proposal') {
        ...
        // Jeśli cena zaakceptowana - pokaż przycisk KUP dla kupującego
        ...
        const accepterUsername = msgData.accepter_username || 'Użytkownik';
        const showBuyButton = !isSeller; // Pokaż przycisk KUP dla kupującego
        ...
        return `
            <div class="negotiation-header">🎉 Transakcja zakończona!</div>
            <div class="negotiation-status status-accepted" style="text-align: center; padding: 20px;">
                <strong>${msgData.buyer_username}</strong> kupił produkt<br>
                <strong>${msgData.product_name}</strong><br>
                za <span style="font-size: 24px;">${parseFloat(msgData.price).toFixed(2)} zł</span>
                <div style="margin-top: 15px; font-size: 14px; color: #666;">
                    💡 Skontaktujcie się w celu ustalenia szczegółów odbioru
                </div>
            </div>
        `;
    }
...
let lastSendTime = 0;
const SEND_COOLDOWN = 5000; // 5 sekund
...
form.addEventListener('submit', async function(e) {
    ...
    if (timeSinceLastSend < SEND_COOLDOWN) {
        ...
        showToast('warning', 'Poczekaj chwilę', `Możesz wysłać wiadomość za ${remainingSeconds} sekund`);
        return;
    }
    ...
    try {
        ...
        if (result.status === 'ok') {
            ...
            showToast('error', 'Błąd', result.message || 'Nie udało się wysłać wiadomości');
        }
    } catch(e) {
        console.error('Błąd:', e);
        showToast('error', 'Błąd', 'Wystąpił problem z wysłaniem wiadomości');
    }
});
